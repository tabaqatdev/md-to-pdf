<script lang="ts">
	...
	function handleTableEdit(tableMarkdown: string, startLine?: number, endLine?: number) {
	...
	function handleTableSave(newMarkdown: string) {
		if (editingTableStartLine !== undefined && editingTableEndLine !== undefined) {
			...
			handleContentChange(updatedContent);
		} else if (editingTableMarkdown && editorContent.includes(editingTableMarkdown)) {
			// Fallback to string replacement
			const updatedContent = editorContent.replace(editingTableMarkdown, newMarkdown);
			handleContentChange(updatedContent);
		}
	...
	function handleResizeMove(event: MouseEvent | TouchEvent) {
		...
		if (i18n.direction === 'rtl') {
			...
			newPercent = ((containerRect.right - clientX) / containerRect.width) * 100;
		} else {
...
<div
	class="bg-background text-foreground relative flex h-screen flex-col print:block! print:h-auto! print:overflow-visible!"
...
>
	<!-- Toolbar -->
	<header
		class="no-print border-border flex h-12 items-center justify-between border-b px-4"
		dir={i18n.direction}
	>
		<div class="flex items-center gap-2">
			...
			<Button variant="ghost" size="icon" onclick={handleNewFile} title={i18n.t.sidebar.newFile}>
		...
		<div class="flex items-center gap-2">
			...
			<div class="bg-muted text-muted-foreground flex items-center rounded-lg border p-1">
				<button
					class="ring-offset-background focus-visible:ring-ring inline-flex items-center justify-center rounded-md px-3 py-1.5 text-sm font-medium whitespace-nowrap transition-all focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 {viewMode ===
					'edit-only'
			...
			<Button variant="ghost" size="icon" onclick={toggleMode} title={i18n.t.theme.title}>
				{#if currentMode === 'dark'}
					<Moon class="h-4 w-4" />
			...
			<Button variant="outline" size="sm" onclick={handleExportPDF} class="hidden sm:flex">
	...
	<!-- Main Content -->
	<main class="flex flex-1 overflow-hidden print:block! print:h-auto! print:overflow-visible!">
		<!-- Sidebar - hidden on mobile when not needed -->
		{#if showSidebar}
			<aside
				class="no-print border-border bg-sidebar hidden w-64 shrink-0 border-e sm:block"
				dir={i18n.direction}
			>
				<Sidebar />
			</aside>
...
{#if isDragging}
	<div
		class="bg-primary/10 pointer-events-none absolute inset-0 z-50 flex items-center justify-center backdrop-blur-sm"
	>
		<div
			class="border-primary bg-background/90 rounded-xl border-2 border-dashed p-8 text-center shadow-lg"
		>
			<FileDown class="text-primary mx-auto mb-4 h-12 w-12" />
