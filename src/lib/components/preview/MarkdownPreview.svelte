<script lang="ts">
  ...
  let headerShowOnFirstPage = $derived(
    settingsStore.current.header.showOnFirstPage
  );
  let footerEnabled = $derived(settingsStore.current.footer.enabled);
  let footerShowPageNumbers = $derived(
    settingsStore.current.footer.showPageNumbers
  );
  let footerPageNumberFormat = $derived(
    settingsStore.current.footer.pageNumberFormat
  );
  ...
  onMount(async () => {
    ...
    const updatePageNumbers = () => {
      const pageNumberElements =
        previewContainer?.querySelectorAll(".page-number");
        ...
        <td>
          {#if headerEnabled}
            ...
            >
              ...
              {/if}
              {#if headerText}
                <span class="header-text" class:ms-4={headerLogo}
                  >{headerText}</span
                >
              {/if}
            </div>
