	...
	<div class="flex-1 overflow-auto">
		...
		{:else if historyStore.isLoading}
			<div class="text-muted-foreground flex items-center justify-center p-4">
				<span class="animate-pulse">{i18n.t.history?.loading || 'Loading...'}</span>
			</div>
		...
		{:else}
			<div class="divide-border divide-y">
				{#each historyStore.versions as version (version.id)}
					<div class="group hover:bg-accent flex items-center justify-between p-3">
						...
						<div class="flex gap-1 opacity-0 group-hover:opacity-100">
							<button
								type="button"
								class="hover:bg-background rounded p-1"
							...
							<button
								type="button"
								class="text-destructive hover:bg-background rounded p-1"
...
{#if showDiff && selectedVersion}
	...
	>
		...
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<div
			class="border-border max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-lg border bg-white shadow-2xl dark:bg-gray-900"
		...
		>
			...
			<div class="border-border flex items-center justify-between border-b p-4">
				<div>
					...
					<p class="text-muted-foreground text-sm">
						{formatDate(selectedVersion.timestamp)} → {i18n.t.history?.current || 'Current'}
					</p>
				</div>
				<div class="flex gap-2">
					<Button variant="outline" size="sm" onclick={() => copyAllChanges('removed')}>
						<Copy class="me-1 h-3 w-3" />
						{i18n.t.history?.copyRemoved || 'Copy Removed'}
					</Button>
					<Button variant="outline" size="sm" onclick={() => copyAllChanges('added')}>
						<Copy class="me-1 h-3 w-3" />
			...
			<div class="max-h-[70vh] overflow-auto p-4 font-mono text-sm">
				{#each diffLines as line, index (index)}
					<div
						class="group flex items-start gap-2 rounded px-2 py-0.5 {line.type === 'added'
							? 'bg-green-500/10 text-green-700 dark:text-green-400'
							: line.type === 'removed'
								? 'bg-red-500/10 text-red-700 dark:text-red-400'
								: ''}"
					>
						<span class="text-muted-foreground w-8 shrink-0 text-end">{line.lineNum}</span>
						<span class="w-4 shrink-0 text-center">
							{#if line.type === 'added'}+{:else if line.type === 'removed'}-{:else}&nbsp;{/if}
						</span>
						<span class="flex-1 break-all whitespace-pre-wrap">{line.content || ' '}</span>
						{#if line.type !== 'unchanged'}
							<button
								type="button"
								class="hover:bg-background shrink-0 rounded p-0.5 opacity-0 group-hover:opacity-100"
			...
			<div class="border-border flex items-center justify-between border-t p-4">
				<div class="text-muted-foreground text-sm">
					<span class="text-green-600">+{diffLines.filter((l) => l.type === 'added').length}</span>
					<span class="mx-2">/</span>
					<span class="text-red-600">-{diffLines.filter((l) => l.type === 'removed').length}</span>
				</div>
