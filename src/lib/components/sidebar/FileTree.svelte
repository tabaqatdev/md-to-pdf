<script lang="ts">
  ...
  let creatingIn = $state<{ path: string; type: "file" | "folder" } | null>(
    null
  );
  ...
  async function confirmCreate() {
    ...
    try {
      if (creatingIn.type === "file") {
        const filePath = await filesStore.createFile(
          creatingIn.path,
          newItemName.trim()
        );
  ...
  async function handleDelete(node: FileNode, event: MouseEvent) {
    ...
    const message =
      node.type === "folder"
        ? i18n.t.confirm.deleteFolder
        : i18n.t.confirm.deleteFile;
  ...
  async function confirmMove() {
    ...
    try {
      const newPath = selectedFolder
        ? `${selectedFolder}/${movingNode.name}`
        : movingNode.name;
      ...
      {#if creatingIn && creatingIn.path === ""}
        <div class="mb-1 flex items-center gap-1 rounded px-2 py-1">
          ...
          <Input
            ...
            placeholder={creatingIn.type === "folder"
              ? "Folder name"
              : "File name"}
...
{#snippet fileNode(node: FileNode, depth: number)}
  ...
  <div class="group">
    ...
    <div
      class="flex w-full items-center gap-1 rounded px-2 py-1 text-sm hover:bg-accent cursor-pointer {isActive
        ? 'bg-accent'
        : ''}"
    ...
    >
      ...
      <div class="hidden gap-0.5 group-hover:flex">
        ...
        <button
          type="button"
          class="rounded p-0.5 text-destructive hover:bg-background"
...
{#if movingNode}
  ...
  >
    ...
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div
      class="w-full max-w-md rounded-lg bg-white dark:bg-gray-900 p-6 shadow-2xl border border-border"
      onclick={(e) => e.stopPropagation()}
    >
      ...
      <div class="mb-4">
        <label for="folder-select" class="block text-sm font-medium mb-2"
